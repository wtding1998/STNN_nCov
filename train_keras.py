...
p.add('--dataset', type=str, help='dataset name', default='jar_rnn')
...
p.add('--val_ratio', type=float, help='time for training', default=0.1)
...
p.add('--rnn_model', type=str, help='choose rnn model : LSTM(GRU)_module | LSTM(GRU)_Linear ', default='GRU_Linear')
# -- optim
...
p.add('--test', type=boolean_string, default=True, help='empty parameter')
# -- seed
...
p.add('--load_exp', type=boolean_string, default=True, help='load_exp')
...
setup, (train_input, train_output), (val_input, val_output), (test_input, test_data)= get_keras_dataset(opt.datadir, opt.dataset, opt.nt_train, opt.seq_length, start_time=opt.start_time, normalize=opt.normalize, reduce=opt.reduce, val_ratio=opt.val_ratio)
...
if opt.load_exp:
    ...
    # exp.quick_show(show=opt.show_fig)
    if opt.run_min:
        exp.run_min_exp()
